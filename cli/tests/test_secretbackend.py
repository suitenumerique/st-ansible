...
AnsibleVaultBackend writes plaintext refs plus an encrypted vault.yml.
HashiVaultBackend is reference-only: lookup refs, no vault.yml, no writes.
"""
...
def test_hashi_lookup_ref_plain_term():
    """A term with no quotes or backslashes is embedded verbatim inside the lookup."""
...
def test_hashi_render_no_marker_is_literal():
    """A value with no marker is kept literal, even for a secret-looking value."""
...
def test_hashi_render_vault_alias_matches_openbao():
    """The @vault(<path>) alias produces an identical result to @openbao(<path>) for the
    same path."""
...
def test_hashi_render_multiple_markers_both_resolve():
    """Multiple markers in one value all resolve; literal text between them is kept
    verbatim."""
...
def test_hashi_render_marker_path_with_quote_is_escaped():
    """A single quote inside a marker path is escaped by hashi_lookup_ref, so the
    quote cannot close the Jinja literal early."""
    ...
    # the marked path's quote survives only as an escaped \' (one quote becomes one \')
    assert out.count("\\'") == 1
...
def test_hashi_env_secret_interpolates_inline_marker(repo, monkeypatch):
    """HashiVaultBackend.env_secret routes the prompted term through hashi_render,
    turning an inline @openbao() marker into a lookup ref in answers."""
    ...
    # reference-only: still no vault buffer, so no vault.yml
    assert backend.component_secrets("meet") == {}


def test_hashi_prompt_term_prefills_openbao_default(monkeypatch):
    """HashiVaultBackend._prompt_term prompts with the bare var name and a
    pre-filled @openbao(kv/data/<app>:<VAR>) default the operator can accept or edit."""
    ...
    # accepting the hint makes hashi_render turn the @openbao() marker into a lookup ref
...
def test_hashi_env_secret_embeds_lookup_term_and_no_vault_yml(repo, monkeypatch):
    ...
    # value would now be kept literal (see
    # test_hashi_env_secret_no_marker_stays_literal).
    monkeypatch.setattr(
    ...
    # no vault buffer, so _write_vault would no-op and write no vault.yml
    assert backend.component_secrets("meet") == {}
...
def test_hashi_env_secret_no_marker_stays_literal(repo, monkeypatch):
    ...
    in answers, with no lookup wrapping."""
    backend = HashiVaultBackend("meet")
...
def test_hashi_shared_provider_secret_single_source(repo, monkeypatch):
    """A secret used as both a provider var and a consumer env ref is prompted
    once, and both sides point at the same OpenBao lookup ref."""
    backend = HashiVaultBackend("meet")
    calls = {"term": 0}
...
def test_ansible_vault_shared_provider_secret_keeps_two_stores():
    """The default shared_provider_secret keeps two stores: raw value in the
    provider buffer, vault_<key> copy in the consumer buffer, and a
...
def test_mirror_livekit_creds_to_egress_separates_buffers():
    """`_mirror_livekit_creds_to_egress` copies livekit's api key and secret into
    egress's own component buffer, under the same `st_meet_livekit_*` var names."""
...
def test_write_common_connection_preserves_existing_keys(repo):
    ...
    preserving pre-existing keys and the header comment block."""
...
def test_setup_backend_reuses_ansible_vault_without_prompt(repo, monkeypatch, capfd):
    ...
    the persisted backend choice silently, with no select prompt."""
    from st_cli.core import prompts
...
def test_setup_backend_reuses_hashi_vault_without_prompt(repo, monkeypatch, capfd):
    """A hashi_vault (app, env) is reused silently on re-bootstrap: no backend
    select, no OpenBao prompt, and no common.yml write."""
    from st_cli.core import prompts
