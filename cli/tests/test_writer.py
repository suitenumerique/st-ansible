...
def test_drive_has_no_legacy_s3_component_vars():
    """drive serves S3 media through the caddy edge (CADDY_S3_* env vars, see
    apps/drive.yml's caddy env_render layer), not through st_drive_s3_*
    ansible vars. A legacy st_drive_s3_host line from a pre-0.4.0 unit still
    survives a replay, because apply_component_vars only ever writes the
    names declared in component_vars and never deletes a committed key."""
    meta = appmeta.load_app("drive")
...
def test_apply_component_vars_keeps_committed_value_when_answer_is_missing():
    """Check that a missing answer does not downgrade a committed value to a literal placeholder."""
    meta = appmeta.load_app("drive")
    data = CommentedMap()
...
def test_apply_component_vars_writes_literal_when_nothing_to_preserve():
    """Check that the literal placeholder is still written when no value is committed."""
    meta = appmeta.load_app("drive")
    data = CommentedMap()
...
def test_vars_header_secrets_line_matches_the_backend():
    ...
    {{ vault_* }} refs that don't exist) would mislead them."""
...
def test_write_core_fresh_unit_behaves_as_before(repo):
    """Check that write_core on a fresh unit renders vars with one header and no merge artifacts."""
    seed_creds(repo)
...
def test_write_core_enter_through_rebootstrap_is_byte_identical(repo):
    """Check that re-running write_core with the same answers reproduces a byte-identical vars.yml."""
    seed_creds(repo)
...
def test_write_core_merge_preserves_custom_var_comment_and_env_line(repo):
    """Check that a rebootstrap preserves a custom var, its comment, and a custom env-blob line."""
    seed_creds(repo)
...
def test_write_core_rebootstrap_updates_value_in_place_and_appends_new_key(repo):
    """Check that a changed answer updates in place and a missing rendered key is appended once."""
    seed_creds(repo)
    answers = {"DOMAIN": "meet.example.org", "DJANGO_ALLOWED_HOSTS": "meet.example.org"}
    _write_meet_core(dict(answers))
...
def test_write_vault_noop_on_empty_buffer_leaves_existing_vault_untouched(repo):
    """Check that an empty component_secrets buffer leaves an existing vault.yml untouched."""
...
def test_write_vault_merges_new_secret_preserving_existing(repo):
    """Check that write_vault merges a new secret into the existing mapping without losing old ones."""
    seed_livekit_provider(repo)
    path = paths.vault_path("meet", "prod", "livekit")
...
def test_write_vault_noop_when_merge_changes_nothing(repo):
    """Check that re-mirroring the same secret value does not rewrite vault.yml (salting would change the diff on every run)."""
    seed_creds(repo)
    backend = AnsibleVaultBackend()
...
def test_write_vault_undecryptable_raises_and_leaves_file_untouched(repo):
    """Check that an undecryptable vault.yml raises StCliError before any write happens."""
    seed_creds(repo)
    path = paths.vault_path("meet", "prod", "livekit")
