...
class _Spy:
    """Replaces a ``questionary.*`` factory and records each call's (args, kwargs)."""
...
def test_ask_validate_replaces_the_required_check(spies):
    text_spy, _ = spies
    text_spy.answer = "https://s3.example.org"

    def check(text):
        return True if text.startswith("https://") else "needs https://"

    prompts._ask("AWS_S3_ENDPOINT_URL", validate=check)
    assert text_spy.kwargs["validate"] is check

    prompts._ask("AWS_S3_ENDPOINT_URL")
    assert text_spy.kwargs["validate"] is prompts._require
