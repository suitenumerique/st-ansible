...
App-specific questionnaire logic (e.g. recomposing a prompt default from a
recovered ``DOMAIN`` value) belongs to the questionnaire itself, not here;
this module only surfaces the raw recovered material for it to compose.
...
def recover(app: str, env: str, component: str) -> dict:
    ...
    or ``app`` is unknown). Otherwise combines three sources:
    ...
    2. **The component-var inversion** (fallback — fills gaps the blob
       ...
       is how ``DOMAIN`` comes back — it is not a literal ``KEY=value`` line
       in any env blob, so the blob alone can never recover it.

    3. **The dotenv component-var inversion** (fallback, line by line —
       fills gaps sources 1 and 2 cannot). When ``tmpl`` is not one whole
       placeholder, it may still be a dotenv-shaped literal block (e.g.
       socks-proxy's ``st_messages_socks_proxy_env``, whose body is
       ``PROXY_EXTERNAL={PROXY_EXTERNAL}\nPROXY_INTERNAL_PORT={PROXY_INTERNAL_PORT}\n...``).
       Both ``tmpl`` and the committed ``data[name]`` are parsed with
       :func:`core.envblob.parse`; for each ``tmpl`` line whose value is
       exactly one placeholder, the same-keyed committed value is recovered
       into ``answers[placeholder]``. A line with embedded text (e.g.
       mta-in's ``MDA_API_BASE_URL=https://{DOMAIN}/...``) has no reliable
       single-answer inverse and is skipped per line.

    A template with embedded text, more than one placeholder, or none at
    all (e.g. meet's ``st_meet_backend_run_migrations``, whose quadrupled
    braces are a literal Jinja expression baked in by the manifest, not a
    questionnaire answer) is **skipped outright** by both inversion passes
    — there is no reliable inverse for those, and guessing would silently
    corrupt the recovered answers.

    **Precedence rule**: a blob-parsed value always wins; neither inversion
    pass overwrites a placeholder the blob already supplied. The two
    inversion passes run interleaved in one loop over ``component_vars``
    order, and the first value recovered for a placeholder wins — no shipped
    manifest recovers one placeholder through both shapes. The blob is the
    literal, verbatim record of what was fed to the Jinja renderer (ground
    truth — :func:`envrender.render_env` is the actual consumer of
    ``answers``), whereas both inversion passes are structural heuristics
    over ``str.format`` templates. In practice the sources never compete for
    the same key (``DOMAIN``/``S3_*``/socks-proxy's ``PROXY_*`` are never
    emitted as env blob ``KEY=`` lines), but if a unit was ever hand-edited
    into disagreement, trusting the blob is the safer default since it is
    what the next render will actually see.
    """
    try:
    ...
    for name, tmpl in meta.component_vars(component).items():
        ...
        if name not in data or not isinstance(data[name], str):
            continue
        # A dotenv-shaped component var (e.g. socks-proxy's st_messages_socks_proxy_env)
        # is a whole literal block, not one placeholder — invert it line by line instead.
        ...
        for key, value in tmpl_lines.items():
            ...
            if not pm:
                continue  # embedded text, e.g. MDA_API_BASE_URL=https://{DOMAIN}/... — skipped
