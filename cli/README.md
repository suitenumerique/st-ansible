...
`.st-cli.yml` pin drift apart, or when a newer upstream release exists, so you
do not have to remember to check by hand:

- installed CLI == pin, and a newer release exists upstream: pull the new
  image (or `pipx upgrade st-cli`). The next command then shows the
...
questionnaire with an old version's templates. It also refuses to run when your
installed CLI is older than the `.st-cli.yml` pin (another operator already ran
`st-cli upgrade` and committed the newer pin): pull the new image first, then
run `st-cli upgrade` again. Both refusals exit with a non-zero status, so a
script or CI job can detect them.
...
one, in a final "Manual steps for this upgrade:" block, one line per app/env,
before the "upgrade complete" line.

A crashed `st-cli upgrade` is safe to re-run: the pin is already aligned, every
unit already replayed is skipped, and each replay is idempotent — it writes
the same files again and never rotates a secret.
...
rebootstrap would. A flag can also mark itself `full_replay: true`, which forces the
...
apps need attention, and `st-cli upgrade` replays them for you (see above):
...
- your installed CLI is older than the `.st-cli.yml` pin — pull the new image
  (or run `pipx upgrade st-cli`) first.
...
Every other pending rebootstrap flag is a warning only, and `deploy` continues
— run `st-cli upgrade` when you get the chance.
...
rebootstrap flags above: a "new env keys available" warning when a template offers a
key your committed blob does not have yet (run `bootstrap` to add it). Your own
keys in a blob are never reported. Neither the advisory, nor a warning-only
rebootstrap flag, ever blocks `deploy`. `deploy` prints them as warnings once
past its gate.
